#' MERMAID boxplots
#'
#' @param .data Sample event data containing mean fish belt biomass, with variables indicated in \code{group_var} and \code{compare_var}. Likely the output from a \code{mermaid_get_project_data()} call.
...
#' }
mermaid_plot_benthic_pit_percent_cover <- function(.data, group_var, compare_var, clean_values = TRUE, clean_values_case = c("title", "sentence"), replace_dashes = TRUE) {
  group_var <- rlang::enquo(group_var)
  group_var_string <- rlang::quo_name(group_var)
  compare_var <- rlang::enquo(compare_var)
  compare_var_string <- rlang::quo_name(compare_var)
  ...
  mermaid_boxplot(.data, group_var, group_var_string, compare_var, compare_var_string, "Mean Percent Cover", clean_values, clean_values_case, replace_dashes)
}
